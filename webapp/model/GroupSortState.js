sap.ui.define([
	...
	return BaseObject.extend("empresa.model.GroupSortState", {
		 ...
		 * @class
		 * @public
		 * @alias empresa.model.GroupSortState
		 */
		...
		/**
		 * Sorts by DESCRIPCION, or by EMPRESAID
		 *
		...
		/**
		 * Groups by EMPRESAID, or resets the grouping for the key "None"
		 *
		 * @param sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function(sKey) {
			...
			if (sKey === "EMPRESAID") {
				...
				this._oViewModel.setProperty("/sortBy", "EMPRESAID");

				aSorters.push(
					new Sorter("EMPRESAID", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "DESCRIPCION");
			}
