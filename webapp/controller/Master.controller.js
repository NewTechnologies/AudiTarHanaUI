/*global history */
sap.ui.define([
	"empresa/controller/BaseController",
	"sap/ui/model/json/JSONModel",
	"sap/ui/model/Filter",
	"sap/ui/model/FilterOperator",
	"sap/m/GroupHeaderListItem",
	"sap/ui/Device",
	"empresa/model/formatter",
	"sap/m/MessageBox",
	"empresa/model/grouper",
	...
	return BaseController.extend("empresa.controller.Master", {
		...
		onInit: function() {
			...
			this._oGroupSortState = new GroupSortState(oViewModel, grouper.EMPRESAID(this.getResourceBundle()));
		...
		onSearch: function(oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("DESCRIPCION", FilterOperator.Contains, sQuery)];
			} else {
		...
		onOpenViewSettings: function() {
			if (!this._oViewSettingsDialog) {
				this._oViewSettingsDialog = sap.ui.xmlfragment("empresa.view.ViewSettingsDialog", this);
		...
		onConfirmViewSettingsDialog: function(oEvent) {
			...
			aFilterItems.forEach(function(oItem) {
				switch (oItem.getKey()) {
					case "Filter1":
						aFilters.push(new Filter("EMPRESAID", FilterOperator.LE, 100));
						break;
					case "Filter2":
						aFilters.push(new Filter("EMPRESAID", FilterOperator.GT, 100));
						break;
					default:
						break;
				}
				...
				function(mParams) {
					...
					if (!mParams.list.getSelectedItem()) {
						this.getRouter().navTo("object", {
							EMPRESAID: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("EMPRESAID"))
						}, true);
					}
				}.bind(this),
				function(mParams) {
		...
		_showDetail: function(oItem) {
			...
			this.getRouter().navTo("object", {
				EMPRESAID: encodeURIComponent(oItem.getBindingContext().getProperty("EMPRESAID"))
			}, bReplace);
		},
